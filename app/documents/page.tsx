...
import { apiProcessDocument, apiFeedback, apiExportUrl, ProcessResponse, KeywordMatch } from "@/lib/api";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Label } from "@/components/ui/label";
import { Upload, CheckCircle, XCircle, HelpCircle, Download, Loader2, FileText } from "lucide-react";
...
interface DocMatchCardProps {
  match: DocMatch;
  onFeedback: (id: string, action: string) => void;
}

function DocMatchCard({ match, onFeedback }: DocMatchCardProps) {
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-2">
            ...
            {match.timestamp_start && (
              <span className="text-xs text-gray-500 font-mono">{match.timestamp_start}</span>
            )}
          </div>
    ...
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-gray-500 mt-1">
          Analiza PDF, TXT y DOCX — muestra referencia de página por detección
        </p>
      </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              >
                ...
                {file ? (
                  ...
                  <p className="text-sm text-gray-500">PDF, TXT, DOCX</p>
                )}
                <input
                  id="file-input-doc"
                  type="file"
                  className="hidden"
                  title="Seleccionar documento PDF, TXT o DOCX"
            ...
            {mutation.isError && (
              <p className="text-sm text-red-600">
                {(mutation.error as Error)?.message || "Error al analizar el documento"}
              </p>
            )}
          </form>
        </CardContent>
