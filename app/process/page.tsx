...
import {
  apiProcess, apiFeedback, apiExportUrl, apiListSessions, apiGetSession,
...
import { Upload, CheckCircle, XCircle, HelpCircle, Download, Loader2, FileAudio, History, Clock, Database } from "lucide-react";
...
function MatchCard({ match, onFeedback }: { match: KeywordMatch & { id?: string }; onFeedback: (id: string, action: string) => void }) {
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-2">
            ...
            <span className="text-xs text-gray-500 font-mono">{match.timestamp_start} → {match.timestamp_end}</span>
          </div>
...
function ResultPanel({ result, file, onFeedback }: { result: ProcessResponse; file: File | null; onFeedback: (id: string, action: string) => void }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {result.total_matches} detecciones · {result.total_review_candidates} para revisar · {result.duration_seconds?.toFixed(1) ?? "—"}s
          </p>
        </div>
        {result.session_id && (
      ...
      {/* Reproductor multimedia con marcadores */}
      {file && result.duration_seconds > 0 && result.matches.length > 0 && (
        <Card>
          <CardHeader><CardTitle className="text-sm">Reproductor con marcadores de detección</CardTitle></CardHeader>
          <CardContent>
            <MediaPlayer file={file} duration={result.duration_seconds} matches={result.matches} sourceType={sourceType} />
          </CardContent>
        </Card>
      )}
      ...
      {result.matches.length > 0 && (
        <Card>
          ...
          <CardContent className="space-y-3">
            {result.matches.map((m, i) => <MatchCard key={i} match={m} onFeedback={onFeedback} />)}
          </CardContent>
        </Card>
      )}
            ...
            {result.review_candidates.map((rc, i) => (
              <div key={i} className="border rounded-lg p-3 bg-yellow-50 dark:bg-yellow-950 border-yellow-200 dark:border-yellow-800">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  ...
                  <span className="text-xs text-gray-500 font-mono">{rc.timestamp_start}</span>
                </div>
      ...
      {result.clips.length > 0 && (
        <Card>
          <CardHeader><CardTitle className="text-sm">Clips generados ({result.clips.length})</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            {result.clips.map((clip, i) => (
              <div key={i} className="border rounded-lg p-3 text-xs">
                <p className="font-mono text-gray-600 dark:text-gray-400">{clip.timestamp_start} → {clip.timestamp_end}</p>
                {clip.error ? <p className="text-red-600 mt-1">{clip.error}</p> : <p className="text-green-700 dark:text-green-400 mt-1">{clip.matches.length} detección(es) en este clip</p>}
              </div>
            ))}
          </CardContent>
        </Card>
...
function HistoryTab() {
  ...
  const loadSession = async (id: string) => {
    setLoadingId(id);
    try {
      const d = await apiGetSession(id);
      setDetail(d);
    } finally {
      setLoadingId(null);
    }
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-gray-500 mt-1">Transcribe y detecta términos relacionados con violencia de género</p>
      </div>
            ...
            <CardContent>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                <div>
                  ...
                  <div className="mt-1 border-2 border-dashed rounded-lg p-8 text-center cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors" onClick={() => document.getElementById("file-input-av")?.click()}>
                    <FileAudio className="h-8 w-8 mx-auto mb-2 text-gray-400" />
                    {file ? <p className="text-sm text-gray-700 dark:text-gray-300 font-medium">{file.name} ({(file.size / 1024 / 1024).toFixed(1)} MB)</p> : <p className="text-sm text-gray-500">MP3, WAV, MP4, MOV... hasta 500 MB</p>}
                    <input id="file-input-av" type="file" className="hidden" title="Seleccionar archivo" accept="audio/*,video/*" onChange={(e) => setFile(e.target.files?.[0] || null)} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label>Idioma</Label>
                    <select {...register("language")} className="mt-1 w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-500">
                  ...
                  <div>
                    ...
                    <select {...register("profile")} className="mt-1 w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-500">
                ...
                <Button type="submit" disabled={!file || mutation.isPending} className="bg-rose-600 hover:bg-rose-700 text-white w-full">
                  {mutation.isPending ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Procesando...</> : <><Upload className="h-4 w-4 mr-2" />Procesar</>}
                </Button>
                {mutation.isError && <p className="text-sm text-red-600">{(mutation.error as Error)?.message || "Error al procesar"}</p>}
              </form>
            </CardContent>
          </Card>
