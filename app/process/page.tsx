...
import { apiProcess, apiFeedback, apiExportUrl, ProcessResponse, KeywordMatch } from "@/lib/api";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Label } from "@/components/ui/label";
import { Upload, CheckCircle, XCircle, HelpCircle, Download, Loader2, FileAudio } from "lucide-react";
...
function MatchCard({ match, onFeedback }: MatchCardProps) {
  const [feedbackSent, setFeedbackSent] = useState<string | null>(null);

  const handleFeedback = (action: string) => {
    if (!match.id) return;
    setFeedbackSent(action);
    onFeedback(match.id, action);
  };

  const colorClass = severityColors[match.severity] || "bg-gray-50 border-gray-200";
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm font-medium mb-1">
            Término: <span className="font-bold">&ldquo;{match.matched_term}&rdquo;</span>
          </p>
          {match.matched_span && (
            <p className="text-xs text-gray-600 italic">&ldquo;{match.matched_span}&rdquo;</p>
          )}
            ...
            {feedbackSent ? (
              ...
              <>
                <Button
                  size="sm"
                  variant="outline"
                  className="h-7 text-xs gap-1 text-green-700 border-green-300"
        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              <div
                className="mt-1 border-2 border-dashed rounded-lg p-8 text-center cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                onClick={() => document.getElementById("file-input-av")?.click()}
              >
                ...
                {file ? (
                  ...
                  <p className="text-sm text-gray-500">MP3, WAV, MP4, MOV... hasta 500 MB</p>
                )}
                <input
                  id="file-input-av"
                  type="file"
      ...
      {result && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            {result.session_id && (
              <a
                href={apiExportUrl(result.session_id)}
                download
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">
                  Detecciones confirmadas ({result.matches.length})
                </CardTitle>
              </CardHeader>
          ...
          {result.review_candidates.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">
                  Candidatos para revisión ({result.review_candidates.length})
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                {result.review_candidates.map((rc, i) => (
                  <div
                    key={i}
                    className="border rounded-lg p-3 bg-yellow-50 dark:bg-yellow-950 border-yellow-200 dark:border-yellow-800"
                  >
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      <Badge className="bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200 text-xs">
                        similitud {(rc.similarity * 100).toFixed(0)}%
                      </Badge>
                      <span className="text-xs text-gray-500 font-mono">{rc.timestamp_start}</span>
                    </div>
                    <p className="text-xs dark:text-gray-300">
                      Detectado: <strong>&ldquo;{rc.detected_word}&rdquo;</strong> → sugerido:{" "}
                      <strong>&ldquo;{rc.suggested_term}&rdquo;</strong>
                    </p>
              ...
              <CardContent className="space-y-2">
                {result.clips.map((clip, i) => (
                  <div key={i} className="border rounded-lg p-3 text-xs">
                    ...
                    {clip.error ? (
                      <p className="text-red-600 mt-1">{clip.error}</p>
                    ) : (
                      <p className="text-green-700 dark:text-green-400 mt-1">
                        {clip.matches.length} detección(es) en este clip
                      </p>
                    )}
                  </div>
                ))}
